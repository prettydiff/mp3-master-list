...
const browser = function ():void {
    const xhr:XMLHttpRequest = new XMLHttpRequest(),
        status:HTMLElement = document.getElementById("load-status") as HTMLElement,
        statusList:HTMLElement = status.getElementsByTagName("ol")[0],
        body:HTMLElement = document.getElementsByTagName("body")[0],
        fileCount:number = Number(body.dataset["jsonCount"]),
        ...
        li = function(text:string):void {
            const item:HTMLElement = document.createElement("li");
            item.textContent = text;
            statusList.appendChild(item);
        },
        html = function(list:directory_list):void {
            li("List JSON data parsed.");
            if (list.length > 0) {
                const play:Element = document.getElementById("player").getElementsByClassName("controls")[0].getElementsByTagName("button")[1].getElementsByTagName("svg")[0],
                    len:number = list.length,
                    headings:HTMLCollectionOf<HTMLElement> = document.getElementsByTagName("thead")[0].getElementsByTagName("th"),
                    heading = function (item:number):string {
                        return headings[index_cell].lastChild.textContent.trim().toLowerCase();
                    },
                    tbody:HTMLElement = document.getElementsByTagName("tbody")[0];
                let index_list:number = 0,
                    index_cell:number = 0,
                    tr:HTMLElement = null,
                    td:HTMLElement = null,
                ...
                li("HTML created from JSON data.");
            }
        },
        complete = function():void {
            const trackList:HTMLElement[] = [],
                list = {
                ...
                tools = {
                    ...
                    randomIndex: function ():number {
                        ...
                        do {
                            ...
                            if (dom.currentTrack === dom.recordsMedia[index]) {
                        ...
                        math_1 = Math.round(seed1 * seed2 * 10 * shown.length);
                        if (shown[math_1] === undefined || math_1 === shown[current]) {
                            math_2 = Math.round(seed1 * seed2 * shown.length);
                            if (math_2 === shown[current]) {
                                return (current === 0)
                                    ? shown[current + 1]
                                    : shown[current - 1];
                            }
        ...
        get = function get(count:number):void {
            xhr.open("GET", location.href.replace(/\.html$/, `_${count}.json`), true);
            xhr.responseType = "text";
            xhr.onload = function ():void {
                li("List data received");
            ...
            li(`Request list data - ${count} of ${fileCount}.`);
            xhr.send();
        };
    get(0);
};
